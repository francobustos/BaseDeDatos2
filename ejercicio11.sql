...
SELECT s.store_id, CONCAT(ci.city, ", ", co.country) as location, 
CONCAT(m.first_name, " ", m.last_name) as manager, sum(amount) as sales
FROM store s
...
GROUP BY a.actor_id
HAVING peliculas_actuadas >= ALL(SELECT COUNT(film_id) FROM film_actor GROUP BY actor_id);
